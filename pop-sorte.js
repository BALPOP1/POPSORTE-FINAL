...
const CONCURSO_REFERENCE = {
    number: 6903,
    date: new Date(2025, 11, 15) // Dec 15, 2025 (month is 0-indexed: 11 = December)
};
...
function buildScheduleForDate(date) {
    ...
    cutoff.setHours(drawHour, 0, 0, 0);
    cutoff.setSeconds(cutoff.getSeconds() - 1); // 19:59:59 or 16:59:59

    const regStart = new Date(drawDate);
    regStart.setDate(regStart.getDate() - 1);
    regStart.setHours(20, 0, 1, 0); // 20:00:01 of previous day
...
function getCurrentDrawSchedule() {
    ...
    const today = new Date(spNow);
    today.setHours(0, 0, 0, 0);

    const todayValid = isValidDrawDay(today);
    if (todayValid) {
        const schedule = buildScheduleForDate(today);
    ...
    const nextDate = getNextValidDrawDate(today.getTime() + 24 * 60 * 60 * 1000);
    const nextSchedule = buildScheduleForDate(nextDate);
...
function calculateConcurso(drawDate) {
    const refDate = new Date(CONCURSO_REFERENCE.date);
...
async function fetchAndPopulateResults() {
    ...
    const updateAndAnimate = (latestResult, winners = []) => {
        ...
        prefix.innerHTML = `Últimos resultados: `;

        // Create/Update suffix
        let suffix = document.getElementById('marqueeSuffix');
        if (!suffix) {
            suffix = document.createElement('span');
            suffix.id = 'marqueeSuffix';
            marqueeBalls.parentNode.insertBefore(suffix, marqueeBalls.nextSibling);
        }
        suffix.innerHTML = drawNumber ? ` (<b>#${drawNumber}</b> - ${formattedDate}) ` : '';
        ...
        if (winners && winners.length > 0) {
            ...
            winnersTitle.innerHTML = '🏆 <b>Ganhadores:</b> ';
            winnersTitle.style.color = '#b45309';
            ...
            winners.forEach((win) => {
                ...
                winTag.style.background = '#fef3c7';
                winTag.style.padding = '3px 10px';
                winTag.style.borderRadius = '12px';
                winTag.style.border = '1px solid #fde68a';
...
async function confirmEntry() {
    ...
    try {
        ...
        const generateTime = spTime.toLocaleTimeString('pt-BR', {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit'
        });
        
        const ticketDateDisplay = drawDate.toLocaleDateString('pt-BR', {
            day: '2-digit',
            month: 'long',
            year: 'numeric'
        });
...
async function sendToTelegram(gameId, whatsappNumber, numeros, drawDate, bilheteNumber) {
    ...
    const message = `
🎫 <b>NOVA PARTICIPAÇÃO POP-SORTE</b>

👤 <b>Game ID:</b> ${gameId}
📱 <b>WhatsApp:</b> ${whatsappNumber}
🎟️ <b>Bilhete:</b> ${bilheteNumber}º bilhete
🎯 <b>Números:</b> ${numeros}
🎰 <b>Concurso:</b> ${concurso}
📅 <b>Sorteio:</b> ${drawDateStr} às ${drawHour.toString().padStart(2, '0')}:00 (BRT)
🕒 <b>Registro:</b> ${new Date().toLocaleString('pt-BR')}
...
function initCountdown() {
    function updateCountdown() {
        ...
        const schedule = getCurrentDrawSchedule();
        const targetTime = new Date(schedule.drawDate);
        targetTime.setHours(schedule.drawHour, 0, 0, 0);
