...
// BRT helpers (UTC-03:00)
const BRT_OFFSET_HOURS = 3;
const BRT_OFFSET_MS = BRT_OFFSET_HOURS * 60 * 60 * 1000;

function brtFields(date) {
    // Shift to BRT (UTC-03) to read UTC components as BRT wall time
...
function makeDateFromBrt(year, month, day, hour = 0, minute = 0, second = 0) {
    // Build an absolute Date for a given BRT wall time
...
class RechargeValidator {
    ...
    // Given a recharge time R, find EligibleDraw1 and EligibleDraw2
    computeEligibleDraws(rechargeTimeObj) {
        if (!rechargeTimeObj) return null;
        const firstDrawDay = this.firstDrawDayAfter(rechargeTimeObj);
        if (!firstDrawDay) return null;
        const eligible1Cutoff = this.buildCutoffDateTime(firstDrawDay);

        // Find next draw day after eligible1
        let probe = addDaysBrt(firstDrawDay, 1);
        let eligible2Day = null;
        for (let i = 0; i < 60; i++) {
            if (!this.isNoDrawDay(probe)) {
                eligible2Day = startOfDayBrt(probe);
                break;
            }
            probe = addDaysBrt(probe, 1);
        }
        if (!eligible2Day) return null;
        const eligible2Cutoff = this.buildCutoffDateTime(eligible2Day);

        return {
            eligible1: { day: firstDrawDay, cutoff: eligible1Cutoff },
